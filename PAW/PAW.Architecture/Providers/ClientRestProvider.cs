...
/// </summary>
public class ClientRestProvider : IRestProvider
{
	/// <summary>
	/// Retrieves a resource asynchronously.
	/// </summary>
	...
	public async Task<string> GetAsync(string endpoint, string? id)
	{
		try
		{
			var response = await ClientRestProviderHelpers.CreateHttpClient(endpoint)
				.GetAsync(id);
			return await ClientRestProviderHelpers.GetResponse(response);
		}
		catch (Exception ex)
		{
			throw ClientRestProviderHelpers.ThrowError(endpoint, ex);
		}
	}
	...
	public async Task<string> PostAsync(string endpoint, string content)
	{
		...
		{
			var response = await ClientRestProviderHelpers.CreateHttpClient(endpoint)
				.PostAsync(endpoint, ClientRestProviderHelpers.CreateContent(content));
			var result = await ClientRestProviderHelpers.GetResponse(response);
			return result;
		}
		catch (Exception ex)
		{
	...
	public async Task<string> PutAsync(string endpoint, string id, string content)
	{
		...
		{
			var response = await ClientRestProviderHelpers.CreateHttpClient(endpoint)
				.PutAsync(id, ClientRestProviderHelpers.CreateContent(content));
			var result = await ClientRestProviderHelpers.GetResponse(response);
			return result;
		}
		catch (Exception ex)
