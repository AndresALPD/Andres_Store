...
public class JsonSerializer
{
	...
	public static async Task<T> DeserializeAsync<T>(byte[] bytes) where T : class
	{
		...
		T? deserialized = await System.Text.Json.JsonSerializer.DeserializeAsync<T>(stream);
		return deserialized!;
	}
	...
	{
		return System.Text.Json.JsonSerializer.Deserialize<T>(content, GetJsonSerializerOptions());
	}
